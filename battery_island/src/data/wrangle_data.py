...
class DataWrangler:
    ...
    - Clean data
        ...
        - Outliers
        - Erroneous values
    - Explore data - via Jupyter Notebook support functions
    ...
    def __init__(self, model_id, grain=None, stage=None) -> object:
        """
        Initialises DataWrangler object.
        :param model_id:
        :param grain:
        :param stage:
        """
    ...
    def _process_data(self, all_batches_dict, stable=True):
        """
        Process raw data into useful files for analysis, machine learning and dashboarding.

        """

        results, cycles_drop_info = self._preprocess_batch(all_batches_dict, return_original_data=False,
                                                           return_cycle_drop_info=True, verbose=True)
        # pprint(cycles_drop_info)
        # describe_results_dict(results)
        return results, cycles_drop_info

    def _preprocess_batch(self, batch_dict, return_original_data=False, return_cycle_drop_info=False,
                          verbose=False):
        """
        ...
        Keyword Arguments:
            return_original_data {bool} -- If True, the original data used for interpolation is returned. (default: {
            False})
        ...
        for cell_key in list(batch_dict.keys()):
            ...
            # Initialise the cell results with all available scalar values.
            ...
            for cycle_key, cycle_value in cell_value["cycles"].items():
                ...
                try:
                    cycle_results = self._preprocess_cycle(cycle_value, return_original_data=return_original_data)
                    ...
                    drop_info = {
                        cell_key: {
                            cycle_key: self._outlier_dict_without_mask(oe.outlier_dict)}}
                    cycles_drop_info.update(drop_info)
                    continue
    ...
    def _preprocess_cycle(self, cycle,
                          I_thresh=-3.99,
                          Vdlin_start=3.5,
                          Vdlin_stop=2.0,
        ...
        Qd, T, V, t = self._multiple_array_indexing(discharge_mask, Qd, T, V, t)

        # Sort all values after time.
        sorted_indeces = t.argsort()
        Qd, T, V, t = self._multiple_array_indexing(sorted_indeces, Qd, T, V, t)

        # Only take timesteps where time is strictly increasing.
        increasing_time_mask = np.diff(t, prepend=0) > 0
        Qd, T, V, t = self._multiple_array_indexing(increasing_time_mask, Qd, T, V, t)

        # Dropping outliers.
        Qd, T, V, t = self._drop_cycle_big_t_outliers(15, Qd, T, V, t)

        Qd = self._handle_small_Qd_outliers(12, Qd, t)

        Qd, T, V, t = self._drop_outliers_starting_left(12, Qd, T, V, t)
        ...
        Qd, T, V, t = self._multiple_array_indexing(v_decreasing_mask, Qd, T, V_savgol, t, drop_warning=True)

        # Make V_3 strictly decreasing (needed for interpolation).
        V_strict_dec = self._make_strictly_decreasing(t, V)
    ...
    def _multiple_array_indexing(self, valid_numpy_index, *args, drop_warning=False, drop_warning_thresh=0.10):
        ...
        if drop_warning:
            self._check_for_drop_warning(args[0], indexed_arrays[0])
        return tuple(indexed_arrays)

    def _check_for_drop_warning(self, array_before, array_after, drop_warning_thresh=0.10):
    ...
    def _outlier_dict_without_mask(self, outlier_dict):
    ...
    def _drop_cycle_big_t_outliers(self, std_multiple_threshold, Qd, T, V, t, t_diff_outlier_thresh=100):
        ...
        If any are found the whole cyce is dropped, with one exception:
            ...
            In this case, all measurement values of Qd, T, V and t after this outlier are dropped and their values
            returned.
        ...
        """
        outlier_dict = self._compute_outlier_dict(std_multiple_threshold=std_multiple_threshold, Qd=Qd, T=T, V=V, t=t)
    ...
    def _drop_outliers_starting_left(self, std_multiple_threshold, Qd, T, V, t):
        ...
        drop_counter = 0
        outlier_dict = self._compute_outlier_dict(std_multiple_threshold, verbose=True, Qd=Qd_, T=T_, V=V_, t=t_)
        ...
        while outlier_dict:
            ...
            Qd_ = self._array_exclude_index(Qd_, unique_indeces_to_drop)
            T_ = self._array_exclude_index(T_, unique_indeces_to_drop)
            V_ = self._array_exclude_index(V_, unique_indeces_to_drop)
            t_ = self._array_exclude_index(t_, unique_indeces_to_drop)

            drop_counter += len(unique_indeces_to_drop)

            # Recompute outlierts after dropping the unique indeces from all arrays.
            outlier_dict = self._compute_outlier_dict(std_multiple_threshold, Qd=Qd_, T=T_, V=V_, t=t_)

        if drop_counter > 0:
            print("    Dropped {} outliers in {}".format(drop_counter, list(original_outlier_dict.keys())))
            print("")

        self._check_for_drop_warning(Qd, Qd_)
        return Qd_, T_, V_, t_

    def _array_exclude_index(self, arr, id):
    ...
    def _handle_small_Qd_outliers(self, std_multiple_threshold, Qd, t, Qd_max_outlier=0.06):
        ...
        outlier_dict = self._compute_outlier_dict(std_multiple_threshold, Qd=Qd_)
    ...
    def _make_strictly_decreasing(self, x_interp, y_interp, prepend_value=3.7):
        """Takes a monotonically decreasing array y_interp and makes it strictly decreasing by interpolation over
        x_interp.
    ...
    def _compute_outlier_dict(self, std_multiple_threshold, verbose=False, **kwargs):
        ...
        if verbose and outlier_dict:
            ...
            outlier_dict_wo_mask = self._outlier_dict_without_mask(outlier_dict)  # Generate a smaller dict for better
            # printing
...
def main():
    ...
    results, cycles_drop_info = data_wrangler.clean_data(data_loader.get_dictionary())
